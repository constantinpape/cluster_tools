...
def master_job(n_jobs):
    ...
    # wait for jobs 1
    failed_jobs = wait_and_check_multiple_jobs('cc_ufd_step1', n_jobs)
    if failed_jobs:
    ...
    # wait for jobs 2
    failed_jobs = wait_and_check_multiple_jobs('cc_ufd_step2', n_jobs)
    if failed_jobs:
    ...
    # wait for jobs 3
    failed_jobs = wait_and_check_single_job('cc_ufd_step3')
    if failed_jobs:
        print("Step 3 failed")
        return
    ...
    # wait for jobs 4
    failed_jobs = wait_and_check_multiple_jobs('cc_ufd_step4', n_jobs)
    if failed_jobs:
