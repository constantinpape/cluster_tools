...
def master_job(n_jobs):
    ...
    # wait for jobs 1
    failed_jobs = wait_and_check_multiple_jobs('masked_watershed_step1', n_jobs)
    if failed_jobs:
    ...
    # wait for jobs 2
    failed_jobs = wait_and_check_multiple_jobs('masked_watershed_step2', n_jobs)
    if failed_jobs:
    ...
    # wait for jobs 3
    failed_jobs = wait_and_check_single_job('masked_watershed_step3')
    if failed_jobs:
        print("Step 3 failed")
        return
    ...
    # wait for jobs 4
    failed_jobs = wait_and_check_multiple_jobs('masked_watershed_step4', n_jobs)
    if failed_jobs:
