...
def seeds_from_connected_components(affs, threshold, exclusion_mask):
    ...
    seeds[exclusion_mask] = 0
    _, max_label, _ = vigra.analysis.relabelConsecutive(seeds, out=seeds, keep_zeros=True)
...
def single_block_watershed(block_id, blocking,
    ...
    # get seeds
    seeds, seed_offset = seeds_from_connected_components(affs_z, threshold_cc, excluded)
    # mask excluded area in the xy affinities
    affs_xy[excluded] = 0
    ...
    # run watershed
    ws, _ = run_watershed(1. - affs_xy, seeds)
...
def masked_watershed_step1(aff_path_xy, key_xy, aff_path_z, key_z,
                           mask_path, mask_key, out_path, key_out,
...
if __name__ == '__main__':
    ...
    masked_watershed_step1(args.aff_path_xy, args.key_xy,
